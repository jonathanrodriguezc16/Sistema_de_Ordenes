...
interface CartDrawerProps {
    ...
    // NUEVA PROP: Para navegar al admin si no hay clientes
    onGoToAdmin?: () => void;
}

export const CartDrawer = ({
    isOpen, onClose, cart, products, total, clients, selectedClientId,
    onSelectClient, onCheckout, onIncrease, onDecrease, onRemove, onGoToAdmin
}: CartDrawerProps) => {
    ...
    return (
        <>
            ...
            {/* Panel */}
            <div className={`fixed top-0 right-0 h-full w-[400px] max-w-[90vw] bg-white shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'
                }`}>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
                    {cart.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-4 opacity-60">
                            ...
                            <button onClick={onClose} className="text-blue-500 text-sm hover:underline">
                                Volver al catálogo
                            </button>
                        </div>
                    ) : (
                        <ul className="space-y-4">
                            {cart.map((item) => {
                                ...
                                return (
                                    <li key={item.productId} className="flex flex-col bg-white p-3 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <div className="flex justify-between items-end bg-gray-50 p-2 rounded-lg">
                                            <div className="flex items-center bg-white rounded-md border border-gray-200 shadow-sm h-8">
                                                ...
                                                <button
                                                    ...
                                                    disabled={isMaxStock}
                                                    className={`px-2 h-full rounded-r-md transition-colors ${isMaxStock
                ...
                <div className="border-t border-gray-100 p-6 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
                    ...
                    <div className="space-y-3">
                        ...
                        {/* LÓGICA: Si no hay clientes, mostramos alerta y botón de crear */}
                        {clients.length === 0 ? (
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2">
                                <div className="flex items-start gap-2 text-amber-800">
                                    ...
                                    <div className="text-xs font-medium leading-relaxed">
                                        No hay clientes registrados en el sistema. Debes crear uno antes de confirmar la orden.
                                    </div>
                                </div>
                                <button
                                    onClick={() => {
                                        onClose(); // Cerramos el carrito
                                        if (onGoToAdmin) onGoToAdmin(); // Vamos al admin
                                    }}
                            ...
                            /* Si hay clientes, mostramos el selector normal */
                            <div className="relative">
                                ...
                                <select
                                    ...
                                    className={`w-full pl-10 pr-4 py-3 border rounded-xl appearance-none outline-none transition-all cursor-pointer ${!selectedClientId
                                        ? 'border-gray-300 text-gray-500 bg-gray-50'
                                        : 'border-blue-200 text-gray-900 bg-blue-50/30 ring-1 ring-blue-100'
                                        }`}
                                >
                                    <option value="" disabled>Seleccionar cliente para el pedido...</option>
                                    {clients.map(c => (
                        ...
                        {/* Botón de Confirmar (Deshabilitado si no hay clientes o carrito vacío) */}
                        <button
                            onClick={onCheckout}
                            disabled={cart.length === 0 || !selectedClientId || clients.length === 0}
                            className={`w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all transform active:scale-[0.99] ${cart.length > 0 && selectedClientId
