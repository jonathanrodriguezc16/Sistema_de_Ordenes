    ...
    if (loading && orders.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-gray-400 bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <p className="text-sm font-medium animate-pulse">Sincronizando historial...</p>
            </div>
        );
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden mb-8 animate-in fade-in duration-500">
            ...
            <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center bg-gray-50/50 gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">Historial de Órdenes</h2>
                        <p className="text-xs text-gray-500 font-medium">
                            {orders.length} {orders.length === 1 ? 'registro encontrado' : 'registros encontrados'}
                        </p>
                    </div>
                </div>
            ...
            <div className="overflow-x-auto">
                {orders.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-20 text-gray-400">
                        ...
                        <p className="text-lg font-medium text-gray-600">No hay órdenes registradas aún</p>
                        <p className="text-sm text-gray-400">Las nuevas ventas aparecerán aquí</p>
                    </div>
                ) : (
                            ...
                            {orders.map((order) => (
                                <tr
                                    ...
                                    className={`
                                        ...
                                        ${order.status === 'cancelled' ? 'bg-gray-50/50 grayscale-[0.5]' : 'bg-white'}
                                    `}
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`text-sm font-bold ${order.status === 'cancelled' ? 'text-gray-400 line-through' : 'text-gray-900'}`}>
...
const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] uppercase tracking-wide font-bold border ${current.styles}`}>
            {current.icon}
            {current.label}
        </span>
    );
